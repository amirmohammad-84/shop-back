...
/* Post Update Product */
router.post('/:id/update' ,authMiddleware , validate(updateProductSchema) ,async (req, res, next) => {
  try {
    const { title , body , category , price } = req.body;
...
// /product/1/delete
router.post('/:id/delete' ,authMiddleware , validate(deleteProductSchema) , (req, res, next) => {
  try {
